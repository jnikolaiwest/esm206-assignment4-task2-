...
ggplot(data = hare_traps_year, aes(x = year_new, y = sample_size)) +
  geom_col(fill="saddlebrown", color = "black", bins = 15) +
  theme_classic() +
...
**Figure 1:** The total number of juvenile snowshoe hares observed each year at the Bonanza Creek Long Term Ecological Research site between `r min(hare_traps_year$year)` and `r max(hare_traps_year$year)`. Data: Keilland et al. (2017).
...
ggplot(data = juv_weight_genders, aes(x = sex, y = weight, fill = sex)) +
  scale_fill_manual(values = c("lightpink2", "lightblue2", "darkseagreen2")) +
  geom_boxplot(outlier.color = "black") +
  geom_beeswarm(shape = 1, alpha = 0.75, aes(color = sex)) +
  scale_color_manual(values = c("gray44", "gray44", "gray44")) +
  facet_wrap(~grid) +
  labs(x = "Hare Gender",
       y = "Weight (g)",
...
```
**Table 1:** Relationship between body weight (g) and gender of juvenile male and female snowshoe hares observed in each study site within the Bonanza Creek Long Term Ecological Research study area between `r min(juv_weight_m_f_only$year)` and `r max(juv_weight_m_f_only$year)`. One deceased hare (gender unknown) was recovered from the Bonanza Spruce site and no weight was taken. Data: Keilland et al. (2017). 
...
**Figure 3:** Linear Relationship between hind foot length (mm) and body weight (g) for juvenile snowshoe hares observed within the Bonanza Creek Long Term Ecological Research study area between `r min(bonanza_hares_juv_genders$year)` and `r max(bonanza_hares_juv_genders$year)`. Points indicate individual hare measurements. Data: Keilland et al. (2017).
...
**Figure 4:** Linear Relationship between hind foot length (mm) and body weight (g) for juvenile snowshoe hares observed within the Bonanza Creek Long Term Ecological Research study area between `r min(bonanza_hares_juv_genders$year)` and `r max(bonanza_hares_juv_genders$year)`. Points indicate individual hare measurements. Linear model summary: $\beta$~1~ = `r round(hares_lm_tidy$estimate[2], 2)` g mm^-1^, p < 0.001, R^2^ = `r round(hares_lm_glance$r.squared,2)`, Pearson's *r* = `r round(hares_cor_tidy$estimate,2)`). Data: Keilland et al. (2017).

The relationship between body weight and hind foot length appears relatively linear, both across and within species (Figure 3). Simple linear regression revealed that Hind foot length significantly predicts body weight (p < 0.001, R^2^ = `r round(hares_lm_glance$r.squared,2)`) with an average slope of $\beta$ = `r round(hares_lm_tidy$estimate[2], 2)` g mm^-1^ (i.e., for each one millimeter increase in hind foot length we expect an average increase in body weight of `r round(hares_lm_tidy$estimate[2], 2)` g). Hind foot length and body weight are strongly, and significantly, positively correlated (Pearson's *r* = `r round(hares_lm_tidy$estimate,2)`, p < 0.001). Diagnostic plots (not included) reveal normally distributed and homoscedastic residuals.  *TO COMPLETE (WHAT WORRIES YOU/ CHECK DATA)*
